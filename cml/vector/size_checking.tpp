 ...
 */
template<class SizeTag1, class SizeTag2> struct checked_binary_size
{
...
template<> struct checked_binary_size<fixed_size_tag, fixed_size_tag>
{
...
template<class Sub, class SizeTag> void checked_minimum_size(
...
template<class Sub, int N> void checked_minimum_size(
  const readable_vector<Sub>& sub, cml::int_c<N>, dynamic_size_tag)
{
  checked_minimum_size(sub, N, dynamic_size_tag());
}
...
template<class Sub, class SizeTag> void checked_size(
  const readable_vector<Sub>& sub, int n, SizeTag)
{
#ifndef CML_NO_RUNTIME_VECTOR_SIZE_CHECKS
...
/** Run-time minimum size checking against a constant. */
template<class Sub, int N> void checked_size(
  const readable_vector<Sub>& sub, cml::int_c<N>, dynamic_size_tag)
{
  checked_size(sub, N, dynamic_size_tag());
}
  ...
  )
{
  ...
  detail::checked_binary_size<tag1,tag2>::check(left.actual(), right.actual());
}
  ...
  )
{
  ...
  detail::checked_binary_size<tag1,tag2>::check(left.actual(), right);
}
...
{
  typedef cml::size_tag_of<Sub> tag;
  detail::checked_minimum_size(left, n, tag());
}
...
{
  typedef cml::size_tag_of<Sub> tag;
  detail::checked_minimum_size(left, cml::int_c<N>(), tag());
}
...
{
  typedef cml::size_tag_of<Sub> tag;
  detail::checked_size(left, n, tag());
}
...
{
  typedef cml::size_tag_of<Sub> tag;
  detail::checked_size(left, cml::int_c<N>(), tag());
}
