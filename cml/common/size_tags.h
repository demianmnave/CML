...
{
  using type = typename T::size_tag;
...
{
  using type = typename traits_of<T>::type::size_tag;
  static_assert(is_size_tag<type>::value, "invalid size tag");
};
...
template<class Sub, class T = void>
using enable_if_fixed_size_t = typename enable_if_fixed_size<Sub, T>::type;
...
template<class Sub, class T = void>
using enable_if_dynamic_size_t = typename enable_if_dynamic_size<Sub, T>::type;
