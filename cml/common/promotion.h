...
template<class Tag1, class Tag2> struct size_check_promote
{
   ...
   */
  using type = cml::if_t<is_any, any_size_tag,
    cml::if_t<is_fixed, fixed_size_tag,
      /* else */ dynamic_size_tag>>;
};

/** Convenience alias for size_check_promote. */
template<class Tag1, class Tag2>
using size_check_promote_t = typename size_check_promote<Tag1, Tag2>::type;
...
template<class Tag1, class Tag2> struct size_tag_promote
{
  ...
  using type = cml::if_t<is_fixed, fixed_size_tag,
...
using size_tag_trait_promote_t = typename size_tag_trait_promote<T1, T2>::type;
...
template<class Tag1, class Tag2> struct basis_tag_promote
{
  ...
  static const bool is_row_basis = is_same_pair<Tag1, Tag2, row_basis>::value
    || is_same_pair<Tag1, Tag2, row_basis, any_basis>::value;

  /* True if possible to promote to col_basis: */
  static const bool is_col_basis = is_same_pair<Tag1, Tag2, col_basis>::value
  ...
  using type = cml::if_t<is_row_basis, row_basis,
...
{
  using type = basis_tag_promote_t<basis_tag_trait_of_t<T1>,
    basis_tag_trait_of_t<T2>>;
};
...
template<class Tag1, class Tag2> struct layout_tag_promote
{
  ...
  static const bool is_row_major = is_same_pair<Tag1, Tag2, row_major>::value
    || is_same_pair<Tag1, Tag2, row_major, any_major>::value;

  /* True if possible to promote to col_major: */
  static const bool is_col_major = is_same_pair<Tag1, Tag2, col_major>::value
  ...
  using type = cml::if_t<is_row_major, row_major,
...
{
  using type = layout_tag_promote_t<layout_tag_trait_of_t<T1>,
    layout_tag_trait_of_t<T2>>;
};
