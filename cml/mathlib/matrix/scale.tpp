...
matrix_scale_along_axis(matrix<E, A, B, L>& m, const VecT& axis, E scale)
{
  ...
  matrix<E, fixed < 3, 3>, B, L > outer_p = outer(axis, axis) * (scale -
    value_type(1));
...
void
matrix_scale_along_axis_2D(matrix<E, A, B, L>& m, const VecT& axis,
  E scale)
{
  typedef matrix<E, A, B, L> matrix_type;
  typedef typename matrix_type::value_type value_type;
  ...
  matrix<E, fixed < 2, 2>, B, L > outer_p = outer(axis, axis) * (scale -
    value_type(1));
  outer_p(0, 0) += value_type(1);
  outer_p(1, 1) += value_type(1);
