...
void
matrix_rotation_align_2D(writable_matrix<Sub>& m,
  const readable_vector<ASub>& align, bool normalize, axis_order2D order)
{
  static_assert(
    cml::are_convertible<value_type_trait_of_t<Sub>,
      value_type_trait_of_t<ASub>>::value,
    "incompatible scalar types");
  ...
  E1 angle_1, E2 angle_2, euler_order order)
{
  ...
  cml_require(0 <= axis && axis <= 2, std::invalid_argument,
...
{
  cml::check_size(euler, cml::int_c<3>());
  matrix_rotation_euler_derivatives(m, axis, euler[0], euler[1], euler[2],
    order);
}
  ...
  const readable_quaternion<QSub>& q)
{
  static_assert(
    cml::are_convertible<value_type_trait_of_t<Sub>,
      value_type_trait_of_t<QSub>>::value,
    "incompatible scalar types");

  using order_type = order_type_trait_of_t<QSub>;
  using q_type = value_type_trait_of_t<QSub>;
  ...
  E& angle, Tol tolerance)
{
  ...
  using asub_type = value_type_trait_of_t<ASub>;

  cml::check_linear_3D(m);
  cml::detail::check_or_resize(axis, int_c<3>());

  /* Assign the axis first: */
  axis.set(m.basis_element(1, 2) - m.basis_element(2, 1),
  ...
  else
  {
    ...
    axis[i] = asub_type(
      value_traits::sqrt(m.basis_element(i, i) - m.basis_element(j, j)
  ...
  E2& angle_2, euler_order order, Tol tolerance, enable_if_matrix_t<Sub>*)
{
  ...
  if(repeat) {
    ...
    if(s1 > tolerance) {
      angle_0 =
        E0(value_traits::atan2(m.basis_element(j, i), m.basis_element(k, i)));
      angle_2 =
        E0(value_traits::atan2(m.basis_element(i, j), -m.basis_element(i, k)));
    } else {
      angle_0 = E0(0);
    ...
    if(c1 > tolerance) {
      angle_0 =
        E0(value_traits::atan2(m.basis_element(j, k), m.basis_element(k, k)));
      angle_2 =
        E2(value_traits::atan2(m.basis_element(i, j), m.basis_element(i, i)));
    } else {
      angle_0 = E0(0);
...
#  define MAT_TEMP_3X3                                                         \
    matrix<typename et::ScalarPromote<typename MatT_1::value_type,             \
             typename MatT_2::value_type>::type,                               \
      fixed<3, 3>, typename MatT_1::basis_orient, row_major>
