...
quaternion_rotation_world_axis(writable_quaternion<Sub>& q, int axis, E angle)
{
  ...
  using order_type = order_type_trait_of_t<Sub>;
  using angle_traits = traits_of_t<E>;
...
void
quaternion_rotation_axis_angle(writable_quaternion<Sub>& q,
  const readable_vector<ASub>& axis, E angle)
{
  static_assert(
    cml::are_convertible<value_type_trait_of_t<Sub>,
      value_type_trait_of_t<ASub>, E>::value,
    "incompatible scalar types");

  using angle_traits = traits_of_t<E>;

  cml::check_size(axis, int_c<3>());
  q.set(angle_traits::cos(angle / E(2)),
    angle_traits::sin(angle / E(2)) * axis);
}

template<class Sub, class MSub>
void
quaternion_rotation_matrix(writable_quaternion<Sub>& q,
  const readable_matrix<MSub>& m)
{
  static_assert(
    cml::are_convertible<value_type_trait_of_t<Sub>,
      value_type_trait_of_t<MSub>>::value,
    "incompatible scalar types");

  using order_type = order_type_trait_of_t<Sub>;
  using value_type = value_type_trait_of_t<Sub>;
  using value_traits = traits_of_t<value_type>;
  using M_type = value_type_trait_of_t<MSub>;
  using M_traits = traits_of_t<M_type>;
  ...
  if(tr >= value_type(0)) {
    ...
    int largest_diagonal_element = index_of_max(m.basis_element(0, 0),
    ...
    const int K = X + k;
    q[I] =
      value_type(M_traits::sqrt(m.basis_element(i, i) - m.basis_element(j, j)
        - m.basis_element(k, k) + value_type(1)))
      / value_type(2);
...
void
quaternion_rotation_euler(writable_quaternion<Sub>& q, E0 angle_0, E1 angle_1,
  E2 angle_2, euler_order order)
{
  ...
  using order_type = order_type_trait_of_t<Sub>;
  using angle0_traits = scalar_traits<E0>;
  using angle1_traits = scalar_traits<E1>;
  using angle2_traits = scalar_traits<E2>;
...
{
  using value_type = value_type_trait_of_t<Sub>;
  using temporary_type = matrix<value_type, compiled<3, 3>>;
...
quaternion_rotation_aim_at(writable_quaternion<Sub>& q,
  ...
  const readable_vector<RSub>& reference, axis_order order)
{
  using value_type = value_type_trait_of_t<Sub>;
  using temporary_type = matrix<value_type, compiled<3, 3>>;
...
void
quaternion_to_axis_angle(const readable_quaternion<Sub>& q,
  writable_vector<ASub>& axis, E& angle, Tol tolerance)
{
  static_assert(
    cml::are_convertible<value_type_trait_of_t<Sub>,
      value_type_trait_of_t<ASub>, E, Tol>::value,
    "incompatible scalar types");

  using angle_traits = scalar_traits<E>;
...
quaternion_to_axis_angle(const readable_quaternion<Sub>& q, Tol tolerance)
{
  static_assert(
    cml::are_convertible<value_type_trait_of_t<Sub>,
      value_type_trait_of_t<VectorT>, Tol>::value,
    "incompatible scalar types");
...
quaternion_to_axis_angle(const readable_quaternion<Sub>& q, Tol tolerance)
{
  using vector_type = vector<value_type_trait_of_t<Sub>, compiled<3>>;
...
void
quaternion_to_euler(const readable_quaternion<Sub>& q, E0& angle_0, E1& angle_1,
  E2& angle_2, euler_order order, Tol tolerance, enable_if_quaternion_t<Sub>*)
{
  static_assert(
...
quaternion_to_euler(const readable_quaternion<Sub>& q, euler_order order,
...
VectorT
quaternion_to_euler(const readable_quaternion<Sub>& q, euler_order order,
  Tol tolerance, enable_if_vector_t<VectorT>*, enable_if_quaternion_t<Sub>*)
{
  using value_type = value_type_trait_of_t<Sub>;
  using temporary_type = matrix<value_type, compiled<3, 3>>;
...
{
  typedef matrix<E, fixed < 3, 3>
  ,
  row_basis, row_major > matrix_type;
...
quaternion_rotate_about_world_axis(quaternion<E, A, O, C>& q, size_t axis,
  E angle)
{
...
template<class E, class A, class O, class C> void
quaternion_rotate_about_local_axis(quaternion<E, A, O, C>& q, size_t axis,
  E angle)
{
  typedef quaternion<E, A, O, C> quaternion_type;
  typedef typename quaternion_type::value_type value_type;
