...
/** Compute the LU decomposition in-place, with L a unit triangular matrix
 * (1's in the diagonal), using Doolittle's method without pivoting.
 *
...
void lu_inplace(MatT& A)
{
  ...
  for(ssize_t k = 0; k < N; ++k) {
    ...
    for(ssize_t j = k; j < N; ++j) {
      value_type sum(0);
      for(ssize_t p = 0; p < k; ++ p) sum += A(k,p)*A(p,j);
      A(k,j) -= sum;
    }
    ...
    for(ssize_t i = k+1; i < N; ++i) {
      value_type sum(0);
      for(ssize_t p = 0; p < k; ++p) sum += A(i,p)*A(p,k);
      A(i,k) = (A(i,k) - sum) / A(k,k);
    }
  }
}

/** Compute the LU decomposition by Doolittle's method, and return a copy
 * of the result.
 *
 * @sa detail::lu_inplace
 */
template<class MatT>
...
/** LU factorization for a matrix, with L a unit lower triangular matrix.
 *
 * @sa detail::lu_copy
 */
...
/** LU factorization for a matrix expression, with L a unit lower
 * triangular matrix.
 *
 * @sa detail::lu_copy
 */
 ...
 * backward substitution.  L is assumed to be a unit lower triangular
 * matrix embedded into LU.
 *
 * @sa lu
 */
...
lu_solve(const MatT& LU, const VecT& b)
{
  ...
  for(ssize_t i = 0; i < N; ++i) {
    value_type yi = b[i];
    for(ssize_t j = 0; j < i; ++j) yi -= LU(i,j)*y[j];
    y[i] = yi;
  }
  ...
  for(ssize_t i = N-1; i >= 0; --i) {
    value_type xi = y[i];
    for(ssize_t j = i+1; j < N; ++j) xi -= LU(i,j)*x[j];
    x[i] = xi/LU(i,i);
  }
