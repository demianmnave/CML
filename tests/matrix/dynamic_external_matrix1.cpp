...
{
  double data[] = {1., 2., 3., 4., 5., 6., 7., 8., 9., 0., 0., 0.};
...
{
  double data[] = {1., 5., 9., 2., 6., 0., 3., 7., 0., 4., 8., 0.};
...
{
  double data[3 * 4];
  double aM[] = {1., 2., 3., 4., 5., 6., 7., 8., 9., 0., 0., 0.};
  cml::externalmnd_c M(data, 3, 4);
  M = aM;
...
{
  double data[3][4] = {{1., 2., 3., 4.}, {5., 6., 7., 8.}, {9., 0., 0., 0.}};
...
{
  double data[4][3] = {{1., 5., 9.}, {2., 6., 0.}, {3., 7., 0.}, {4., 8., 0.}};
...
{
  double data[3 * 4];
  double aM[3][4] = {{1., 2., 3., 4.}, {5., 6., 7., 8.}, {9., 0., 0., 0.}};
  cml::externalmnd_c M(data, 3, 4);
  M = aM;
  // Note: external<> copies arrays using row-major layout.
...
CATCH_TEST_CASE("size_check1")
{
  ...
  CATCH_REQUIRE(M.cols() == 4);
  CATCH_REQUIRE(M.data() == &data[0]);
  CATCH_REQUIRE_THROWS_AS((M = {1., 2., 3., 4., 5., 6., 7., 8., 9.}),
