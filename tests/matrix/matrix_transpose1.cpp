...
{
  auto M = cml::matrix33d(1., 2., 3., 1., 4., 9., 1., 16., 25.);

  M.transpose();
  auto expected = cml::matrix33d(1., 1., 1., 2., 4., 16., 3., 9., 25.);

  for(int i = 0; i < 3; ++i)
    for(int j = 0; j < 3; ++j) CATCH_CHECK(M(i, j) == expected(i, j));
}
...
CATCH_TEST_CASE("fixed transpose_3x3_1")
{
  ...
  for(int i = 0; i < 3; ++i)
    for(int j = 0; j < 3; ++j) CATCH_CHECK(T(i, j) == expected(i, j));
}

CATCH_TEST_CASE("fixed transpose_3x3_2")
{
  auto T = cml::transpose(cml::matrix33d(1., 2., 3., 1., 4., 9., 1., 16., 25.));
...
{
  auto M = cml::matrixd(3, 3, 1., 2., 3., 1., 4., 9., 1., 16., 25.);
...
{
  auto T =
    cml::transpose(cml::matrixd(3, 3, 1., 2., 3., 1., 4., 9., 1., 16., 25.));
